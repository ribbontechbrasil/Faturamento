...
<head>
  ...
  <style>
    ...
    * {{ box-sizing: border-box; }}
    ...
    #btnExportItens:hover {{ background: rgba(31,122,76,.2); }}
    .chip {{
...
<body>
  <div class="wrap">
    <header class="hero">
      ...
      <p class="no-print">Dashboard interativo de faturamento, custo e lucro. A impressão é mês a mês: marque o mês e gere o PDF daquele mês.</p>
      <p class="print-only print-meta" id="printMeta"></p>
      <div class="period" id="periodLabel">Base completa: {periodo_label}</div>
      <div class="hero-actions no-print">
        <button class="btn-print" id="btnImprimir" type="button">Imprimir relatório do mês</button>
      </div>
      <div class="kpi-grid">
    ...
    <section id="secCaixaMes">
      <div class="section-head">
        <div>
          <h2>Caixa mês a mês</h2>
          <p>Venda líquida − despesas ADM − investimento, acumulado desde jul/2026. O cartão Caixa é a última linha.</p>
        </div>
      </div>
    ...
    <section class="filters">
      ...
      <div class="filter-grid">
        ...
        <div class="filter-span-2">
          <label>Mês (um por vez na impressão · mais recentes primeiro)
            <div class="check-box" id="fMesBox"></div>
            <div class="check-actions">
              <button type="button" id="btnMesAll">Marcar todos (só na tela)</button>
      ...
      <div class="filter-actions">
        ...
        <button class="btn-print" id="btnImprimirFiltros" type="button">Imprimir relatório do mês</button>
      ...
      <p class="hint">Dica: o dashboard abre no mês mais recente. A impressão sai <strong>sempre de um mês</strong> (o marcado; se houver vários, o mais recente). O segmento Ativo fica fora da análise.</p>
    ...
    <section>
      <div class="section-head">
        ...
        <div class="filter-actions" style="margin:0;">
          <button class="btn-ghost btn-icon" id="btnExportItens" type="button" title="Exportar lucro por item para Excel">
            <svg width="16" height="16" viewBox="0 0 24 24" aria-hidden="true" focusable="false">
              <path fill="currentColor" d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8l-6-6zm1 7V3.5L18.5 9H15zM8.2 18l2.1-3.2L8.3 12h1.7l1.2 1.9 1.2-1.9h1.6l-2 3 2.1 3.1h-1.7l-1.3-2-1.3 2H8.2z"/>
            </svg>
            Excel
          </button>
  ...
  <script>
    ...
    function computeCaixaAcumulado() {{
      ...
      for (const m of months) {{
        const liq = liqByMonth.get(m) || 0;
        const opex = opexByMonth.get(m) || 0;
        const inv = invByMonth.get(m) || 0;
        const resultado = liq - opex;
        const caixaMes = resultado - inv;
        caixa += caixaMes;
        serie.push({{ m, liq, opex, inv, resultado, caixaMes, caixa }});
      }}
      return {{ caixa, serie }};
    }}
    ...
    function renderCaixaMes(serie) {{
      const tb = document.getElementById('tblCaixaMes');
      if (!tb) return;
      const rows = serie || [];
      tb.innerHTML = rows.map((s, idx) => {{
        const last = idx === rows.length - 1;
        let resultadoTxt = money(s.resultado);
        if (s.inv) {{
          resultadoTxt = `${{money(s.resultado)}} − ${{money(s.inv)}} = ${{money(s.caixaMes)}}`;
        }}
        const resClass = (s.caixaMes || 0) < 0 ? 'neg' : 'pos';
        const cxClass = (s.caixa || 0) < 0 ? 'neg' : 'pos';
        return `
        <tr class="${{last ? 'caixa-row-total' : ''}}">
          <td>${{fmtMonthLabel(s.m)}}</td>
          <td>${{money(s.liq)}}</td>
          <td>${{money(s.opex)}}</td>
          <td>${{s.inv ? money(s.inv) : money(0)}}</td>
          <td class="${{resClass}}">${{resultadoTxt}}</td>
          <td class="${{cxClass}}"><strong>${{money(s.caixa)}}</strong></td>
        </tr>`;
      }}).join('') || `<tr><td colspan="6">Sem dados de caixa a partir de jul/2026.</td></tr>`;
    }}
    ...
    function exportItensExcel() {{
      const filters = readFilters();
      const rows = sortedItens(applyFilters(filters));
      if (!rows.length) {{
    ...
    function latestMonthValue() {{
      const inputs = [...document.querySelectorAll('input[name="mes"]')];
      return inputs.length ? inputs[0].value : null;
    }}

    function monthForPrint() {{
      const selected = checkedValues('mes').slice().sort().reverse();
      if (selected.length >= 1) return selected[0];
      return latestMonthValue();
    }}

    function updatePrintButtonLabel() {{
      const mes = monthForPrint();
      const label = mes
        ? `Imprimir relatório de ${{fmtMonthLabel(mes)}}`
        : 'Imprimir relatório do mês';
      ['btnImprimir', 'btnImprimirFiltros'].forEach(id => {{
        const el = document.getElementById(id);
    ...
    function currentPrintMeta(filters) {{
      const mes = (filters.meses && filters.meses[0]) || monthForPrint();
      const chips = [];
      if (mes) chips.push(`Mês ${{fmtMonthLabel(mes)}}`);
      if (filters.segmentos && filters.segmentos.length) chips.push(`Segmento: ${{filters.segmentos.join(', ')}}`);
      if (filters.cliente) chips.push(`Cliente: ${{filters.cliente}}`);
      if (filters.uf) chips.push(`UF: ${{filters.uf}}`);
      if (filters.material) chips.push(`Material: ${{filters.material}}`);
      ...
      return `Relatório mensal impresso em ${{quando}}. ${{extra}}Um mês por vez · KPIs, gráficos e todas as linhas das tabelas.`;
    }}
    ...
    function beginPrintMode() {{
      ...
      if (period) {{
        if (!state.periodDefault) state.periodDefault = period.textContent;
        period.textContent = mes ? `Mês: ${{fmtMonthLabel(mes)}}` : (state.periodDefault || period.textContent);
      }}
      const filters = readFilters();
    ...
    function endPrintMode() {{
      ...
      if (state.printRestore) {{
        const set = new Set(state.printRestore.meses || []);
        const inputs = [...document.querySelectorAll('input[name="mes"]')];
        if (set.size) {{
          inputs.forEach(i => {{ i.checked = set.has(i.value); }});
        }}
        state.printRestore = null;
      }}
      const period = document.getElementById('periodLabel');
      if (period && state.periodDefault) period.textContent = state.periodDefault;
      clearChartSnapshots();
      refresh();
    }}
    ...
    function clearFilters() {{
      ...
      document.querySelectorAll('input[name="seg"], input[name="despcat"]').forEach(i => {{ i.checked = false; }});
      const mesInputs = [...document.querySelectorAll('input[name="mes"]')];
      mesInputs.forEach((i, idx) => {{ i.checked = idx === 0; }});
