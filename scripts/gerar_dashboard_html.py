  ...
  <script>
    ...
    function renderClientes(rows, filters) {{
      ...
      if (meta) {{
        ...
        meta.textContent = `NFs do filtro atual por cliente. Clique na linha para filtrar; clique de novo ou em “Ver todos” para limpar${{faixaTxt}}${{limTxt}}.`;
      }}
      ...
      tb.innerHTML = ranked.map(c => `
        <tr class="item-row ${{clienteAtivo === c.nome ? 'active' : ''}}" data-cliente="${{c.nome.replaceAll('"', '&quot;')}}">
          <td>${{c.nome}}</td>
          <td class="nfs-cell" title="${{c.nfsTxt.replaceAll('"', '&quot;')}}">${{c.nfsTxt || '—'}}</td>
          <td>${{money(c.venda)}}</td>
          <td class="${{c.liq < 0 ? 'neg' : 'pos'}}">${{money(c.liq)}}</td>
          <td class="${{(c.lucroPct ?? 0) < 0 ? 'neg' : 'pos'}}">${{pct(c.lucroPct)}}</td>
        </tr>
      `).join('') || `<tr><td colspan="5">Nenhum cliente na faixa ${{CLIENTE_FAIXA_LABEL[faixaCli] || 'selecionada'}}.</td></tr>`;
