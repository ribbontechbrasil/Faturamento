#!/usr/bin/env python3
"""NF 1167 item rot.PEAD86x165T3: custo R$ 602,40 e venda líquida R$ 4,82."""
...
def test_override_direto():
    ...
    assert venda_liquida_override("1167", "ro.PEAD86x165T3") == NF_1167_LIQ
    assert venda_liquida_override(1167, "ETBOPP100x80") is None
    assert venda_liquida_override(1176, "rot.PEAD86x165T3") is None
...
def test_planilha_agosto_item():
    ...
    liq = float(row.iloc[0]["Venda líquida"])
    ...
    assert approx(liq, NF_1167_LIQ), liq
    assert not approx(liq, NF_1167_LIQ_ERRADA_T, tol=1.0), liq
...
if __name__ == "__main__":
    ...
    print("OK: NF 1167 rot.PEAD86x165T3 custo R$ 602,40 · venda líquida R$ 4,82")
